...
import client.R;
import client.fragment.discover.DiscoverContactCommentFragment;
import util.RxBus;
import util.Util;
...
public class PopContactDiscoverPerson extends PopBaseView implements View.OnClickListener {
    ...
    public void onClick(View view) {
        if (view == tv_update_money) {
            ...
            Util.sms(ctx, "18515982882", "小李你真是个大傻子");
            showComment();
        }else if(view==tv_call_comment){
            //TODO 联系之后去评价了
            RxBus.get().post("addFragment",new AddFragmentBean(new DiscoverContactCommentFragment()));
            dismiss();
        }
    }
