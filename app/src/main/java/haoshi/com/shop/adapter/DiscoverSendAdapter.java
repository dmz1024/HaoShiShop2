...
public class DiscoverSendAdapter extends BaseAdapter<DiscoverBean.Data> {
    ...
    public DiscoverSendAdapter(Context ctx, ArrayList<DiscoverBean.Data> list, boolean isSend) {
        super(ctx, list);
        this.isSend = isSend;
    }
...
//                initRecycle(recyleViewImageHolder.rv_content, data.data5);
    ...
    public class DiscoverBaseViewHolder extends BaseViewHolder {
        ...
        protected void itemOnclick(int id, final int layoutPosition) {
            ...
            switch (id) {
                case R.id.tv_delete:
                    DiscoverController.getInstance().cancelSend(list.get(layoutPosition).list.goodsId, new OnSingleRequestListener<SingleBaseBean>() {
                        @Override
        ...
        protected void onClick(int layoutPosition) {
            ...
            DiscoverBean.Data.ListBean data = list.get(layoutPosition).list;
            if (isSend) {
                SendBean sendBean = new SendBean();
                sendBean.setId(data.goodsId);
                sendBean.setDesc(data.shareContent);
                sendBean.setImg(data.gallery.get(0));
                sendBean.setName(data.goodsName);
                RxBus.get().post("sendSendRxBus", sendBean);
                RxBus.get().post("back", "back");
            } else {
                RxBus.get().post("addFragment", new AddFragmentBean(DiscoverDescFragment.getInstance(data.goodsId)));
            }
