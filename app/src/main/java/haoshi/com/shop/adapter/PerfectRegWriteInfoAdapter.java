...
public class PerfectRegWriteInfoAdapter extends BaseAdapter<PerfectRegInfoTagBean.Data> {
    public PerfectRegWriteInfoAdapter(Context ctx, ArrayList<PerfectRegInfoTagBean.Data> list) {
        super(ctx, list);
    }
                        ...
                        new PopEdit(ctx,true,viewHolder.et_content.getText().toString()){
                            ...
                            protected void content(String content) {
                                ...
                                if(TextUtils.isEmpty(content)){
                                    ...
                                    list.get(position).result = list.get(position).value + "!===!" + content;
                                }

                                viewHolder.et_content.setText(content);
                            }
                        }
                    ...
                    public void onClick(View view) {
                        new ChooseStringView<PerfectRegInfoTagBean.ChooseListBean>(ctx, list.get(position).list) {
                            ...
                            protected void itemClick(int p) {
                                ...
                                list.get(position).result = "{\"fid\":\"" + list.get(position).list.get(p).fid + "\",\"tid\":\"" + list.get(position).list.get(p).tid + "\"}";
            ...
            public void onClick(View view) {
                new PopChooseArea<MyAreaBean, MyAreaListBean>(ctx) {
                    ...
                    protected void name(String area_name) {
                        ...
                        et_content.setText(list.get(position).chooseContent);
                        notifyDataSetChanged();
                    }

                    @Override
                    protected void id(String area_id) {
                        String id = area_id.split("-")[1];
                        list.get(position).result = "{\"fid\":\"" + id + "\",\"tid\":\"" + list.get(position).tid + "\"}";
                    }
